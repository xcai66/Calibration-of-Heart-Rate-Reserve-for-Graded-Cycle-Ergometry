...
const repeatedAgreement = await parseJson("reviewer_round5_weee_agreement.json");
...
const repeatedAgreementByScore = Object.fromEntries(repeatedAgreement.agreement.filter((row) => row.device === "zephyr").map((row) => [row.score, row]));
...
body += `<rect x="65" y="610" width="820" height="455" rx="18" fill="${C.pale}" stroke="${C.grid}"/><text class="panel" x="90" y="653">C  Zephyr-to-reference repeated-measures agreement</text>`;
...
rows.forEach(([key, label, color], index) => {
  const value = repeatedAgreementByScore[key];
  const y = 750 + index * 80;
  const x = (number) => iccL + Math.max(0, Math.min(1, number)) * (iccR - iccL);
  body += `<text class="label" x="300" y="${y + 7}" text-anchor="end">${label}</text><line x1="${x(value.participant_balanced_ccc_ci_low)}" y1="${y}" x2="${x(value.participant_balanced_ccc_ci_high)}" y2="${y}" stroke="${color}" stroke-width="7"/><circle cx="${x(value.participant_balanced_ccc)}" cy="${y}" r="10" fill="${color}"/><text class="small" x="${x(value.participant_balanced_ccc_ci_high) + 10}" y="${y + 7}">${value.participant_balanced_ccc.toFixed(3)}</text>`;
});
body += `<text class="small" x="90" y="1010">Participant-balanced concordance (CCC); cluster-bootstrap 95% CIs.</text><text class="small" x="90" y="1038">tHRR-I bias −0.025; repeated-measures limits −0.262 to 0.213 HRR units.</text>`;
...
const name = "Figure_5_external_construct_validation";
